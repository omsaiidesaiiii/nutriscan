...
export default async function AnalyticsPage() {
  ...
  const proteinPct = totalCaloriesWeek > 0 ? Math.round((proteinCalories / totalCaloriesWeek) * 100) : 0
  ...
  return (
    <div className="space-y-8 pb-8 animate-in">
      ...
      <div className="space-y-1">
        ...
        <p className="text-sm text-zinc-500">Your nutritional performance over the last 7 days.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="card p-6">
          <span className="text-sm font-medium text-zinc-500">Avg. Daily Calories</span>
          <div className="flex items-baseline space-x-1 mt-2">
            <span className="text-3xl font-semibold text-zinc-900">{avgCalories}</span>
        ...
        {/* Avg Protein */}
        <div className="card p-6">
          <span className="text-sm font-medium text-zinc-500">Avg. Daily Protein</span>
          <div className="flex items-baseline space-x-1 mt-2">
            <span className="text-3xl font-semibold text-zinc-900">{avgProtein}</span>
        ...
        <div className="card p-6">
          <div className="flex items-center space-x-2 text-zinc-500 mb-2">
            ...
            <span className="text-sm font-medium">Daily {isDeficit ? 'Deficit' : 'Surplus'}</span>
          </div>
          <div className="flex items-baseline space-x-1">
            <span className="text-3xl font-semibold text-zinc-900">{Math.abs(dailyDifference)}</span>
            <span className="text-sm text-zinc-400">kcal</span>
          </div>
          <p className="mt-2 text-xs text-zinc-400">
            {daysWithData.length > 0 
              ? `Based on ${daysWithData.length} day${daysWithData.length !== 1 ? 's' : ''} of data` 
              : 'No data yet'}
          </p>
        </div>
      </div>
          ...
          <div className="card p-6">
            <div className="flex items-center justify-between mb-5">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-base font-semibold text-zinc-900">Consistency</h3>
              </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-zinc-400">{rating} · Based on ±10% target adherence</p>
            </div>
          </div>

          {/* Macro Split */}
          <div className="card p-6">
            <h3 className="text-base font-semibold text-zinc-900 mb-5">Macro Split</h3>
            
            {totalCaloriesWeek > 0 ? (
              <div className="space-y-5">
                {[
                  { label: 'Protein', val: proteinPct, color: 'bg-emerald-600' },
                  { label: 'Carbs (est.)', val: carbsPct, color: 'bg-emerald-400' },
              ...
              <p className="text-sm text-zinc-400 text-center py-4">No data this week yet.</p>
            )}
          </div>
        </div>
      </div>
